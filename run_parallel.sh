#!/usr/bin/env bash
# =============================================================================
# Run N Firecracker VMs in parallel, each executing the same task
#
# This script:
...
cleanup() {
  ...
  for tap in "${CREATED_TAPS[@]+"${CREATED_TAPS[@]}"}"; do
    # Remove iptables rules for this TAP from FC_FWD chain
    local default_iface
    default_iface="$(ip route list default | awk '{print $5; exit}' || true)"
    if [[ -n "${default_iface}" ]]; then
      iptables -D "${FC_FWD_CHAIN}" -i "${tap}" -o "${default_iface}" -j ACCEPT 2>/dev/null || true
      iptables -D "${FC_FWD_CHAIN}" -i "${default_iface}" -o "${tap}" -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT 2>/dev/null || true
    fi
...
INSTANCE_LOG_DIR=$(mktemp -d -t fc-parallel-XXXXXX)
...
# (tap0 already exists from install_firecracker.sh)
# ---------------------------

log "Setting up TAP devices"
echo "  tap0 already exists (from install_firecracker.sh)"
...
for i in $(seq 1 $((NUM_INSTANCES - 1))); do
  ...
  # Add forwarding rules to the existing FC_FWD chain (idempotent)
  iptables -C "${FC_FWD_CHAIN}" -i "${TAP}" -o "${DEFAULT_IFACE}" -j ACCEPT 2>/dev/null \
    || iptables -A "${FC_FWD_CHAIN}" -i "${TAP}" -o "${DEFAULT_IFACE}" -j ACCEPT
  iptables -C "${FC_FWD_CHAIN}" -i "${DEFAULT_IFACE}" -o "${TAP}" -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT 2>/dev/null \
    || iptables -A "${FC_FWD_CHAIN}" -i "${DEFAULT_IFACE}" -o "${TAP}" -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
...
log "Launching ${NUM_INSTANCES} Firecracker instances in parallel"
