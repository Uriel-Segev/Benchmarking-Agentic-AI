#!/usr/bin/env bash
# =============================================================================
# Run linear scaling benchmark: execute a task at VM counts increasing by step
#
# Runs run_parallel.sh at increments of STEP (default 10) up to MAX_VMS
...
#
# Examples:
#   sudo ./run_scaling_linear.sh /opt/cloud-hypervisor/rootfs-task.ext4 100 --label 48c --repeats 5
#   sudo ./run_scaling_linear.sh /opt/cloud-hypervisor/rootfs-task.ext4 80 --label 48c --step 5
# =============================================================================
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  --step <n>            Increment between VM counts (default: 10)"
      exit 0
      ;;
    *)
      POSITIONAL+=("$1")
