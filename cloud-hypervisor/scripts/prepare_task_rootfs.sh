#!/usr/bin/env bash
# =============================================================================
# Prepare a Cloud Hypervisor rootfs for running terminal-bench tasks
#
# This script:
...
# IMPORTANT: Cloud Hypervisor has ACPI support, so use "poweroff" instead of
# "reboot -f" (which would restart the VM instead of exiting the process).
...
EOF
sync
# Cloud Hypervisor has ACPI — "poweroff" cleanly exits the VMM process.
# Do NOT use "reboot -f" here (that would restart the VM, not exit).
poweroff
AUTORUN_SCRIPT
